...
def print_wheel_velocity_ctrlranges(mjmodel: MjModel) -> None:
    """
    Print loaded ctrl ranges for wheel velocity actuators.
    """
    for actuator_name in ("left_wheel_vel", "right_wheel_vel"):
        actuator_id = mujoco.mj_name2id(mjmodel, mujoco.mjtObj.mjOBJ_ACTUATOR, actuator_name)
        if actuator_id == -1:
            print(f"Actuator '{actuator_name}' not found in loaded model.")
            continue

        ctrl_low, ctrl_high = mjmodel.actuator_ctrlrange[actuator_id]
        print(f"Loaded ctrlrange for {actuator_name}: [{ctrl_low:.6g}, {ctrl_high:.6g}]")
